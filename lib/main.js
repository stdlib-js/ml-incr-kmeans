...
function incrkmeans() {
	...
	if ( isMatrixLike( arguments[ 0 ] ) ) {
		...
		if ( !isPositiveInteger( ndims ) ) {
			throw new TypeError( format( '0LR52', ndims ) );
		}
		...
		throw new TypeError( format( '0LR53', arguments[ 0 ] ) );
	}
	opts = {
		'metric': 'euclidean',
	...
	*/
	function accumulator( vec ) {
		...
		if ( v.shape[ 0 ] !== ndims ) {
			throw new Error( format( '0LR56', ndims, v.shape[ 0 ] ) );
		}
	...
	*/
	function predict( out, X ) {
		...
		if ( o === void 0 ) {
			...
			throw new Error( format( '0LR5A', x.shape[ 0 ], o.length ) );
		}
		if ( init ) {
			return null;
		}
