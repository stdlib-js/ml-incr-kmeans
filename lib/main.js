...
function incrkmeans() {
	...
	if ( isMatrixLike( arguments[ 0 ] ) ) {
		...
		if ( !isPositiveInteger( ndims ) ) {
			throw new TypeError( format( '0h34p,Pk', ndims ) );
		}
		...
		throw new TypeError( format( '0h34q,Pl', arguments[ 0 ] ) );
	}
	opts = {
		'metric': 'euclidean',
	...
	if ( opts.init[ 1 ] < k ) {
		throw new RangeError( format( '0h34r,Pm', 'init', opts.init[ 1 ] ) );
	}
	...
	*/
	function accumulator( vec ) {
		...
		if ( !isVectorLike( v ) ) {
			throw new TypeError( format( '0h3I2', v ) );
		}
		if ( v.shape[ 0 ] !== ndims ) {
			throw new Error( format( '0h34t,Pn', ndims, v.shape[ 0 ] ) );
		}
	...
	*/
	function predict( out, X ) {
		...
		if ( arguments.length > 1 ) {
			if ( !isVectorLike( out ) ) {
				throw new TypeError( format( '0h3Po', out ) );
			}
			o = out;
			x = X;
		} else {
		...
		if ( !isMatrixLike( x ) ) {
			throw new TypeError( format( '0h3Pp', x ) );
		}
		if ( x.shape[ 1 ] !== ndims ) {
			throw new Error( format( '0h3Pq', ndims, x.shape[ 1 ] ) );
		}
		if ( o === void 0 ) {
			o = createVector( x.shape[ 0 ], false ); // high-level
		} else if ( o.length !== x.shape[ 0 ] ) {
			throw new Error( format( '0h34x,Pr', x.shape[ 0 ], o.length ) );
		}
		if ( init ) {
			return null;
		}
